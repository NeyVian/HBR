<#
    H.B.R - Hearthstone Battlegrounds Reconnector
    ...
    Shoutout to Eviatar Gerzi (@g3rzi) for Invoke-DropNet function
#>

#Port TCP to trigger the reconnect process
$HS_TcpPort = 3724

#Window position from right-bottom corner
$Offset_Right = 20
$Offset_Bottom = 40
...
<# User interface #>
    ...
    $closeButton.Location = New-Object System.Drawing.Point(180,0)
    ...
    $left = $screen.WorkingArea.Width - $form.Width - $Offset_Right
    $top = $screen.WorkingArea.Height - $form.Height - $Offset_Bottom
    $form.Location = New-Object System.Drawing.Point($left, $top)

    $form.Add_Shown({$form.Focus()})
    $form.ShowDialog()
}
...
if (-not (amI-Admin))
{
    Write-Host "Needs Administrator privileges"
}
else
{
    Set-Interface
